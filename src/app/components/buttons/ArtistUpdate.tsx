...
const ArtistUpdate: React.FC = () => {
    ...
    const handleArtistOpenModal = () => {
        setIsArtistModalOpen(true);
    }

    const handleArtistCloseModal = () => {
    ...
    const onArtistSubmit = async (data: artistUpdateFormData) => {
        const requestBody = {
            ...data,
          };

        try {
            const response = await fetch('/api/artistUpdates', {
              method: 'POST',
              headers: {
            ...
            if (response.ok) {
              ...
              throw new Error(responseData.error || 'Artist Update Form failed')
            }
          } catch (error: unknown) {
                    ...
                    <div className="modal-content relative w-auto max-w-3xl mx-auto my-6" ref={modalRef}>
                        <div className="relative flex flex-col w-full bg-black text-brand-green-dark border-0 rounded-lg shadow-lg outline-none focus:outline-none">
                            <div className="flex items-start justify-between p-5 border-b border-solid border-brand-green-dark rounded-t">
                                <h3 className="text-3xl text-brand-green-dark font-semibold">Subscribe for Artist Updates</h3>
                                <button
                                    ...
                                    onClick={handleArtistCloseModal}
                                >
                                    <span className="text-brand-green-dark k h-6 w-6 text-2xl block outline-none focus:outline-none">x</span>
                                </button>
                            </div>
                            ...
                            {isSubmitSuccessful && (
                                <span className="text-green-500">
                                    Thank you for subscribing to artist updates!
                                </span>
                                )}
